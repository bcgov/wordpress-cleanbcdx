...
CMD_RESULTS=$(rclone lsf :s3:clbcdx/oc-sites-bk --s3-provider Other --s3-access-key-id "nr-cleanbcdx-pr" --s3-secret-access-key "$S3_TOKEN" --s3-endpoint "https://nrs.objectstore.gov.bc.ca" -P --stats-log-level NOTICE --stats 60s | head -n $BACKUP_NUMBER | tail -n $BACKUP_NUMBER)
